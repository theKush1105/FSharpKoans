   ...
   *)
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            xs |> List.filter(fun v -> (v % 2) > 0) // write a function to filter for odd elements only.
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            List.fold (fun sum elem -> sum + elem) initialState xs // write a function to do what's described above
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            List.fold (fun mult elem -> mult * elem) initialState xs // write a function to multiply the elements of a list
