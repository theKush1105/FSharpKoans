...
module ``03: Putting the Function into Functional Programming`` = 
    ...
    let ``12 You can write a function as a one-liner (Part 1).`` () =
        (fun first -> fun second -> first * second) 9 3 |> should equal 27
...
*)
    ...
    *)
    ...
    let ``34 Partially specifying arguments (Part 3).`` () =
        ...
        let cows =
            fun noise ->
                noise + " de gozaru" // <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
